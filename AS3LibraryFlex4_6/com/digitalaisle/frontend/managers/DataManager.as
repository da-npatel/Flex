package com.digitalaisle.frontend.managers
{
	...
	public class DataManager extends EventDispatcher
	{
		...
		private var _disableProjectContent:Vector.<int>= new Vector.<int>();
		...
		private var _disableProjectContentItem:Dictionary=new Dictionary();
		...
		public function get disableProjectContentItem():Dictionary
		{
			return _disableProjectContentItem;
		}

		public function set disableProjectContentItem(value:Dictionary):void
		{
			_disableProjectContentItem = value;
		}
		...
		/**Fetch all products and Disable product also**/
		...
		public function fetchAll():ArrayCollection
		{
			...
			for(var i:int = 0; i < _disableProjectContent.length; i++) {
				var item:ProjectContentItem = _disableProjectContentItem[_disableProjectContent[i]]
				if(item){	
					if(item is ProjectProductItem) {
						products.addItem(_disableProjectContentItem[_disableProjectContent[i]]);
					}
				}
			}
			return products;
		...
		private function processProjectContentItems(projectContentItem:XMLList, ownerId:int):void
		{
			...
			for(var i:int = 0; i < projectContentItem.length(); i++) {
				if(projectContentItem[i].contentItem.status == ACTIVE) {
					...
					//trace(projectItem.isDeleted)
				   if(projectItem.isDeleted==0){
					_projectContentItemsDict[projectItem.id] = projectItem;
					_allProjectContentItems.push(projectItem.id);
				   }else{
					   _disableProjectContentItem[projectItem.id]=projectItem;
					  _disableProjectContent.push(projectItem.id);
				   }
